...
| **Служба**  | **Опис**  |
| ------ | ---- |
| [Автентифікація](#authentication-events) | Ця служба дає змогу ввійти в Office, активувати ліцензію Office, отримати доступ до файлів, які зберігаються в хмарі, а також забезпечує узгоджені умови роботи всіх сеансів Office і ваших пристроїв.    |
...
Ця подія записується, коли обліковий запис із припустимим маркером оновлення знайдено серед списку облікових записів, які надає служба TokenSharingManager. Цей сценарій характерний для єдиного входу (SSO).
...
Ця подія записується, коли сталася помилка під час спроби отримати маркер оновлення для облікового запису у списку облікових записів, які надає TokenSharingManager. Цей сценарій характерний для єдиного входу (SSO)
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office завантажує нові файли Office. Використовуються для визначення успішності інсталяції Office.
...
Збираються такі поля:
  ...
  - **Data\_ScenarioName -** який сценарій виконується (наприклад, інсталяція). 
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office створює шрифти. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office створює параметри для коректної роботи оновлень. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office завершує інсталяцію, закріплює ярлики, створює записи реєстру для додатків Office. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office переносить параметри з попередніх версій Office. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office створює віртуальні записи реєстру для рівня віртуалізації AppV. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли інсталятор Office видаляє попередню версію Office, інстальовану зі Store. Використовуються для визначення успішності інсталяції Office.
...
Дані настроювання та обліку Office, що збираються, коли клієнт виправлення Office завантажує останню версію клієнта для технології "Office умить", щоб підготувати комп’ютер до видалення й повторної інсталяції програми.
...
Дані настроювання та обліку Office, що збираються, коли клієнт відновлення Office намагається виправити деякі записи реєстру з відомими проблемами. Використовується для визначення успішності відновлення Office.
...
Дані настроювання та обліку Office, що збираються, коли клієнт виправлення Office видаляє Office із пристрою для підготовки до перевстановлення під час виправлення. Використовуються для визначення успішності відновлення Office.
...
Дані настроювання та обліку Office, що збираються, коли клієнт "Office умить" за потреби оновлює ліцензії. Дають змогу визначити, чи було оновлення Office успішним.
...
Дані настроювання та обліку Office, що збираються, коли клієнт "Office умить" завантажує нове оновлення. Використовуються для визначення успішності інсталяції Office.
...
Звіти про дію, яка зчитує позначені тегами вбудовані підписи exe. Це неперевірена концепція, яка не була впроваджена в попередній ітерації setup.exe, і на яку ми спираємося при перенесенні виборів користувача щодо продукту, мови чи розрядності з веб-сторінки до процесу в setup.exe.
...
Телеметрія критичного використання збирається, коли для користувача відображається елемент керування Paywall. Використовується, щоб зрозуміти процес здійснення внутрішніх покупок користувачем і оптимізувати його в майбутніх версіях.
...
Ця подія фіксує кліки користувачів без передплати для перегляду функції за межами платного доступу. Ці дані використовуються, щоб вимірювати взаємодію користувачів із контекстуальними додатковими продажами, а також зрозуміти, яким функціям надає перевагу користувач, і що схиляє його до придбання передплати. Це допоможе нам і надалі покращувати ці бажані точки входу. 

Збираються перелічені нижче поля:

- **featureId** — TCID для преміум-функції

- **featureName** — назва преміум-функції

- **seePlanButtonClick** — скільки разів натиснуто кнопки «Переглянути план» в інтерфейсі користувача додаткового продажу

### <a name="officeappleiapreviewyoursubscriptionios"></a>Office.Apple.IAPReviewYourSubscriptioniOS

Ця подія записує метадані на основі сеансу, коли користувачу відображається інтерфейс користувача внутрішньої покупки (IAP) і кнопки для подальшої взаємодії. Ці дані використовуються, щоб допомогти нам зрозуміти конфлікти в процесі придбання та порівняти їх із лійковими діаграмами інших способів придбання та знайти оптимальний варіант для користувача. 

Збираються перелічені нижче поля:

- **FlowType** — ціле число — потік від точки запуску IAP.

- **Restore** — рядок — тег правила записується при натисканні кнопки відновлення

- **PremiumFeatures** — рядок — тег правила записується при натисканні кнопки «Преміум-функції»

- **Product** — рядок — вибрані користувачами облікові номери.


### <a name="officeappleinapppurchasecontext"></a>Office.Apple.InAppPurchaseContext

Ця подія вимірює телеметрію критичного використання для точки входу на екрані покупки через програму. Дані допомагають зрозуміти та покращити взаємодію з користувачем, визначаючи бажану точку входу для покупки через програму.

Збираються перелічені нижче поля:

- **context** — рядок — потік, який привів користувача на сторінку покупки через програму.


### <a name="officeapplelicensingcommonpaywallcontrol"></a>Office.Apple.Licensing.CommonPaywallControl

Ця подія використовується, щоб зрозуміти процес здійснення внутрішніх покупок зі сторони покупця. Вона допомагає нам переконатися, що внутрішні покупки працюють належним чином, а також зрозуміти проблеми користувача, щоб ми могли оптимізувати роботу з внутрішніми покупками.  Збір даних відбувається за допомогою однієї з указаних нижче субподій.

- **Office.iOS.Paywall.Paywall.Presented** – дані збираються, коли елементи керування системою платного доступу відображаються користувачу. Вони використовуються, щоб виміряти коефіцієнт конверсії на кожному етапі та переконатися, що інтерфейс користувача працює належним чином, а під час покупок у користувачів виникає мінімум проблем.
...
- **Office.iOS.Paywall.Paywall.Stats** –дані збираються, коли користувачу відображається інтерфейс платного доступу, тривалість взаємодії та інформація, чи було здійснено спробу придбання й чи успішно вона завершилася. Дані використовуються, щоб виміряти продуктивність інтерфейсу користувача та переконатися, що він працює належним чином. 
   ...
   - **isFRE** – логічне значення – перевірте, чи відображається перший запуск, чи звичайний інтерфейс користувача.
   - **status** – рядок – стан виходу із системи платного доступу. Наприклад, "initiated", "paymentDone", "provisionFailed".
   - **userDuration** – подвійне значення – час у мілісекундах, який користувач провів у системі платного доступу
  
- **Office.iOS.Paywall.SKUChooser.BuyButtonTap** – дані збираються, коли користувач натискає кнопку "Придбати". Вони використовуються, щоб виміряти продуктивність кнопки та переконатися, що вона працює належним чином.

   Збираються перелічені нижче поля.

   - **entryPoint** – рядок – кнопка або процедура, за допомогою яких було відображено платний доступ. Наприклад, "Кнопка оновлення преміум-класу" або "Процедура першого запуску".
   - **isDefaultSKU** – логічне значення – якщо користувач придбає продукт, ми радимо йому цей продукт за замовчуванням.
   - **productId** – рядок – ідентифікатор продукту в магазині App Store, для якого натиснуто кнопку "Придбати".
...
- **Office.iOS.Paywall.SKUChooser.Stats** – дані збираються, щоб дізнатися, як користувач увійшов у систему вибору облікових номерів, скільки часу він у ній провів і чому вийшов. За допомогою цієї інформації ми можемо переконатися, що система вибору облікових номерів працює належним чином, а також оптимізувати та покращити взаємодію з кінцевим користувачем.
   ...
   - **isFRE** – логічне значення – відображається перший запуск чи звичайний інтерфейс користувача?
   - **userDuration** – подвійне значення – час у мілісекундах, який користувач провів у системі вибору облікових номерів.

- **Office.iOS.Paywall.FailedScreen.RetryButtonTap** – дані збираються, коли не вдалося виконати придбання, ініціалізацію чи активацію й користувач натиснув кнопку "Спробувати знову". Вони використовуються, щоб усунути несправності в сценаріях помилок під час придбання та виправити їх для належної роботи.
...
- **Office.iOS.Paywall.SKUChooser.MoreBenefits.Stats** – дані збираються, коли користувачі натискають кнопку "Інші переваги", щоб переглянути всі служби, програми та функції, що входять до складу покупки. Вони мають розгорнути розділи з інформацією про функції кожної програми. Ця подія збирає інформацію про те, які функції та програми вони розгорнули, і витрачений на це час. Дані використовуються, щоб переконатися, що інтерфейс, який пропонується користувачу для ознайомлення з перевагами, працює належним чином. 

   Збираються перелічені нижче поля.

   - **appsExpanded** – рядок – розділений комами список служб і програм, для яких було розгорнуто інформацію про переваги.
   - **productId** — рядок – ідентифікатор продукту в App Store, для якого користувач переглядає додаткові переваги.
   - **userDuration** – подвійне значення – час у мілісекундах, який користувач витратив на перегляд переваг.

- **Office.iOS.Paywall.SuccessScreen.SeeAllBenefitsButtonTap** – ця подія збирається, коли користувач натискає кнопку "Переглянути всі переваги" після придбання, щоб переглянути програми та функції, які входять до складу покупки. Дані використовуються, щоб переконатися, що інтерфейс користувача працює належним чином.

   Збираються перелічені нижче поля.

   - **productId** – рядок – ідентифікатор продукту в App Store, для якого користувач переглядає всі переваги.

- **Office.iOS.Paywall.SKUChooser.ProductSwitched** – збирається телеметрія використання, щоб переглянути взаємодію кінцевого користувача з інтерфейсом, який дає змогу перемикатися між різними обліковими номерами, та переконатися, що він працює належним чином. 

   Збираються перелічені нижче поля.

  - **productId** – рядок – ідентифікатор продукту в App Store, на перегляд якого користувач щойно перемкнувся з доступних продуктів у засобі вибору облікових номерів.

- **Office.iOS.Paywall.StoreKit.Response** – критично важлива інженерна телеметрія для реєстрації в журналі результату спроби придбання, яку користувач ініціював вручну, і відповіді магазину App Store на цю подію. Дані використовуються, щоб визначити стан спроби придбання та причини невдачі (якщо є) і вжити корегувальні заходи, аби переконатися, що внутрішні покупки та всі точки входу працюють належним чином.

   Збираються перелічені нижче поля.

   - **entryPoint** – рядок – кнопка або процедура, за допомогою яких було відображено платний доступ. Наприклад, "Кнопка оновлення преміум-класу" або "Процедура першого запуску".
   - **failureReason** – рядок – додається лише якщо є стан помилки. Позначає відповідь на помилку, яку надав магазин App Store.
   ...
   - **status** – рядок – "Успішно" або "Помилка", що вказує на успішність або невдачу запиту.

- **Office.iOS.Paywall.Provisioning.Response** – критично важлива інженерна телеметрія та контракт із Retail Federation Service (RFS) для збору інформації, наданої в цій події. RFS – це внутрішня служба, яка використовується корпорацією Майкрософт для перехресної перевірки покупок. Використовується, щоб отримати інформацію про стан виклику API до RFS, яка допоможе зрозуміти, чи працює інтегрування належним чином.  
...
Ця подія збирає дані про натискання користувачами без передплати преміум-функції за межами платного доступу. Ці дані допомагають нам дізнатися про набір функцій, з якими взаємодіють користувачі, коли оформляють передплату. Так ми зможемо зрозуміти, яким точкам входу надають перевагу користувачі, і покращити досвід користувачів.

Збираються перелічені нижче поля:

- **featureId** — TCID для преміум-функції, яку натискає користувач
...
Ця подія запускається під час спроби активувати постійну пропозицію Office на основі пристрою для користувача. Ми використовуємо ці дані, щоб відстежувати справність систем і служб.

Збираються перелічені нижче поля: 

- **Activity_Success** – повідомляє, чи пристрій ліцензовано за допомогою постійної пропозиції Office на основі пристрою.

- **Data_Count** – повідомляє кількість постійних прав office на основі пристрою, пов'язаних із пристроєм. Технічно не повинно бути більше одного.

- **Data_EligibleEntitlementsCount** – повідомляє кількість відповідних прав. Оскільки служба поверне всі права на використання пристрою, нам потрібно перевірити пропозиції, що стосуються запущеної програми Office.

- **Data_Errors** – рядок із розділеним комами списком помилок під час отримання ліцензій на права.

- **Data_LicensedEntitlementsCount** – повідомляє кількість умов обслуговування, для яких успішно отримано ліцензію. Можуть виникати помилки умов обслуговування, які призводять до того, що ми не зможемо отримати ліцензію. 


### <a name="officelicensingactivation"></a>Office.Licensing.Activation 

Після налаштування ліцензії на комп’ютері ми намагаємось активувати ліцензію, викликавши службу AVS. Повідомляється результат дзвінка активації
...
Ця подія запускається один раз для кожного типу ліцензування інформаційного рядка, який відображатиметься з активним тестовиком динамічного інформаційного рядка (експериментальна група). Дані цієї події допомагають зрозуміти, чи є вже на диску кампанія динамічного інформаційного рядка платформи програмування життєвого циклу. Дані використовуватимуться, щоб виміряти продуктивність нової технології ліцензування динамічного інформаційного рядка платформи програмування життєвого циклу.

Збираються перелічені нижче поля:

- **DoesCampaignExist (bool)** — указує, чи кампанія на диску

- **Тип (int32)** — тип ліцензування інформаційного рядка.


### <a name="officelicensingbusbarshowstashedbusbar"></a>Office.Licensing.BusBar.ShowStashedBusbar

Ця подія запускається, коли не вдається відобразити динамічний інформаційний рядок платформи програмування життєвого циклу, а натомість потрібно відобразити відкладений статичний інформаційний рядок. Дані цієї події використовуватимуться, щоб переконатися в успішному переході на статичний інформаційний рядок. 
...
Ця подія сигналізує, що процес покупки в додатку були закриті програмою або користувачем. Дані використовуються для моніторингу та оповіщення про справність потоку внутрішніх покупок, щоб забезпечити його належну роботу.  
...
Ця подія сигналізує про спробу завантаження процесу здійснення внутрішніх покупок, час очікування якої вичерпано, але сталася помилка, в результаті якої діалогове вікно не відображалося. Дані використовуються для моніторингу та оповіщення про справність потоку внутрішніх покупок, щоб забезпечити його належну роботу.  
...
Ця подія сигналізує, що процес покупки в додатку завантажується у фоновому режимі. Дані використовуються для моніторингу та оповіщення про справність потоку внутрішніх покупок, щоб забезпечити його належну роботу.  
...
Ця подія сигналізує, що користувачеві демонструється досвід покупки в додатку. Ці дані використовуються для відстеження та оповіщення про стан потоку покупок в додатку.  
...
Дані цієї події збираються під час відображення діалогового вікна зі сповіщенням про завершення терміну дії ліцензії. Це дуже важливо, щоб визначити, чи перебуває користувач у належному стані та чи всі функції йому доступні. Використовується для забезпечення справності системи й виконання діагностики, якщо користувач повідомляє про проблему, що виникла в роботі комп’ютера.
...
Ця подія запускається при отриманні ліцензії для обраного користувача дозволу Office під час першого запуску активації. Ми використовуємо ці дані, щоб відстежувати справність систем і служб.

Збираються перелічені нижче поля:

- **Activity_Success** - Boolean: повідомляє нам, чи успішно ми отримали ліцензію для пристрою, який буде активовано в його додатку Office.

- **Data_AllowNULPerpetual** - Boolean: повідомляє нам, чи увімкнена тестова версія для включення vNext Perpetual.

- **Data_AttemptNulReactivation** - Boolean: повідомляє нам, чи це сценарій реактивації.

- **Data_CurrentMode**- 0 означає SPP (застарілий стек ліцензування), 2 означає vNext (сучасний стек ліцензування).

- **Data_HasError** - Boolean: повідомляє нам, чи виникли у нас будь-які помилки при спробі отримати ліцензію для обраного дозволу користувача.

- **Data_IsSubscription** - Boolean: повідомляє нам, чи призначена повторна активація для офісу підписки.

- **Data_NewMode** - 0 означає SPP (застарілий стек ліцензування), 2 означає vNext (сучасний стек ліцензування).  Ми повинні очікувати 2 більшу частину часу.

- **Data_SkuToSkuNeeded** - Boolean: повідомляє нам, чи потрібно нам перетворювати SKU в SKU через те, що назва Office SKU не відповідає встановленому Office SKU.
...
- **InAFOFlow** – це логічне значення, яке вказує на те, чи перебуваємо ми в потоці активації Office.
...
Ця подія запускається, коли ми активуємо пропозицію Office для користувача, який придбав пристрій із попередніми правами на використання ПО Office або ввів ключ продукту. Ми використовуємо ці дані, щоб відстежувати справність систем і служб.

Збираються перелічені нижче поля:

- **Activity_Result_Tag** – повідомляє, як ми завершили цю подію.

- **Data_DialogResult** – повідомляє загальний результат процесу активації.

- **Data_Scenario** – повідомляє сценарій активації.
...
Якщо ми закриваємо або припиняємо роботу Office через проблеми з ліцензуванням, ми відправляємо цей сигнал, щоб вказати про це. Це дуже важливо, щоб визначити, чи перебуває користувач у належному стані та чи всі функції йому доступні. Ця подія використовується для забезпечення справності системи й виконання діагностики, якщо користувач повідомляє про проблему, що виникла в роботі комп’ютера.
...
Ця подія ініціюється, коли під час перевірки служби (див. Office.Licensing.OOBE.SearchForDigitalAttach) на цьому пристрої не знайдено заявлену пропозицію "Цифрове вкладення". Користувачу показуватимуться різні діалогові вікна залежно від стану пристрою. Ця подія реєструє в журналі різні сценарії того, як ми обробляємо невдалу спробу пошуку цифрового вкладення.

Збираються перелічені нижче поля.

- **Activity_Result_Tag** повідомляє, як ми переводимо користувача до різних станів помилки.
   - 0x222e318f – потрібно продовжити пошук пропозиції "Активація для Office".
   - 0x222e318e – у цьому сеансі ми повернемось у режим OEM, якщо з пристроєм не надається пропозиція "Цифрове вкладення".
   - 0x222e318d – немає підключення до Інтернету. Користувачу відобразиться діалогове вікно "NoInternetConnectivity" 
   - 0 – ми показуватимемо користувачу різний інтерфейс помилок залежно від конкретного коду помилки.

- **Data_DigitalAttachErrorType** – повідомляє нам конкретний код помилки, повернутий викликом служби.

- **Data_FallbackFlight** – повідомляє нам стан тестовика "UseAFOAsFallBack" (увімкнуто чи вимкнуто).


### <a name="officelicensingoobehandledigitalattachsuccess"></a>Office.Licensing.OOBE.HandleDigitalAttachSuccess

Ця подія ініціюється, коли під час перевірки служби на цьому пристрої знайдено заявлену пропозицію "Цифрове вкладення". Користувачу показуватимуться різні діалогові вікна залежно від стану пристрою. Ця подія реєструє в журналі різні сценарії того, як ми обробляємо успішну спробу пошуку цифрового вкладення.

Збираються перелічені нижче поля.

- **Activity_Result_Tag** – повідомляє нам, як ми обробляємо сценарії успішного пошуку пропозиції "Цифрове вкладення".
   - 0 – ми змогли автоматично завантажити ідентичність і показали користувачу повідомлення "У вас є Office" (з обліковим записом).
   - 0x222e3191 – нам не вдалось автоматично завантажити ідентичність, тому ми показали повідомлення "У вас є Office" (без облікового запису).
   - 0x222e3193 – ми показали користувачу повідомлення "У вас Office" (без облікового запису), або нам не потрібно показувати жодне таке повідомлення, оскільки це пропозиція для певних пристроїв.

- **Data_IsClaimTypeDevice** – повідомляє нам, чи тип твердження пропозиції цифрового вкладення залежить від пристрою.
...
Виробники оригінального обладнання (OEM) продають комп’ютери, які постачаються разом із програмою Office (з передплатою на один рік або весь час), плата за яку враховується під час покупки комп’ютера. Ця подія відстежує, коли для пристрою знайдено попереднє право на використання Office і користувач уже виконав вхід за допомогою облікового запису Microsoft, щоб ми могли відстежувати справність системи та служб.

Збираються перелічені нижче поля.

- **Data_ExpirationDate**– повідомляє нам дату завершення терміну дії пропозиції передплати

- **Data_IsSubscription** – повідомляє нам, який обліковий номер має продукт, що підлягає отриманню: передплачений чи постійний

- **Data_ProductName** – повідомляє нам назву продукту пропозиції цифрового вкладення


### <a name="officelicensingoobesearchfordigitalattach"></a>Office.Licensing.OOBE.SearchForDigitalAttach

Виробники оригінального обладнання (OEM) продають комп’ютери, які постачаються разом із програмою Office (з передплатою на один рік або весь час), плата за яку враховується під час покупки комп’ютера. Комп’ютери, що постачаються з певним ключем реєстрації (OOBEMode: OEMTA), можуть містити пропозицію Office як цифрове вкладення. Під час завантаження Office ми виконуємо перевірку служб, щоб дізнатися, чи знайдено пропозицію Office, вкладену цифровим способом. Ця дія призначена для такого відстеження. 

Збираються перелічені нижче поля.

- **Activity_Result_Tag** – повідомляє нам загальний результат перевірки служб. 
   - 0x222e318c – тестовик "Цифрове вкладення" вимкнуто; перевірка служб не виконується.
   - 0x222e318b – на клієнті немає інтернет-з’єднання, тому перевірка служб не виконується.
   - 0x222e318a – знайдено пропозицію "Цифрове вкладення", яку можна активувати.
   - 0x222e3189 – знайдено пропозицію "Цифрове вкладення", яку не можна активувати.

- **Data_EnableDAFlight** – повідомляє нам стан (увімкнуто або вимкнуто) тестовика "Цифрове вкладення", який активує перевірку служб.


### <a name="officelicensingoobeshowtouchlessattachfailuredialog"></a>Office.Licensing.OOBE.ShowTouchlessAttachFailureDialog

Виробники оригінального обладнання (OEM) продають комп’ютери, які постачаються разом із програмою Office (з передплатою на один рік або весь час), плата за яку враховується під час покупки комп’ютера. Ця подія ініціюється, коли під час активації пропозиції "Цифрове вкладення" на ПК виробників оригінального обладнання, що постачаються з попереднім правом на використання Office, виникає помилка.  Ми використовуємо ці дані, щоб відстежувати справність систем і служб та виправляти помилки, пов’язані з потоком активації Office (OEM).

Збираються перелічені нижче поля.

- **Data_Continue** – повідомляє нам, чи натиснув користувач кнопку "Продовжити" в діалоговому вікні.

- **Activity_Result_Tag** – повідомляє нам, яку кнопку натиснув користувач у діалоговому вікні.
   - 0x222e319d – у діалоговому вікні користувач натиснув кнопку "Спробувати знову"
   - 0x222e319c – у діалоговому вікні користувач натиснув кнопку "Продовжити"
   - 0 – користувач закрив діалогове вікно

- **Data_IsForDigitalAttach** – повідомляє нам, на якій платформі та на якому робочому циклі перебуває користувач: застаріла версія (Активація для Office (AFO)) або сучасна версія (Цифрове вкладення).

- **Data_Retry** – повідомляє, чи натиснув користувач кнопку "Спробувати знову" в діалоговому вікні.


### <a name="officelicensingoobeshowtouchlessattachofferdialog"></a>Office.Licensing.OOBE.ShowTouchlessAttachOfferDialog

Виробники оригінального обладнання (OEM) продають комп’ютери, які постачаються разом із програмою Office (з передплатою на один рік або весь час), плата за яку враховується під час покупки комп’ютера. Ця подія відстежує, коли для пристрою знайдено попереднє право на використання Office, але користувач не виконав вхід за допомогою облікового запису Microsoft, щоб ми могли відстежувати справність системи та служб.

Збираються перелічені нижче поля.

- **Activity_Result_Tag** – повідомляє, чи знайдено ідентичність користувача
   - 0x222e3194 – не вдалось отримати ідентичність користувача (скасовано вхід або не вдалося виконати автентифікацію).
   - 0 – користувач надав ідентичність.

- **Data_ExpirationDate**– повідомляє нам дату завершення терміну дії пропозиції передплати

- **Data_IsCentennial** – повідомляє, чи програма Office працює на платформі Centennial

- **Data_IsForDigitalAttach** – повідомляє, який потік ініціював це діалогове вікно: "Цифрове поєднання" чи "Активація для Office".

- **Data_IsSubscription** – повідомляє нам, який обліковий номер має продукт, що підлягає отриманню: передплачений чи постійний

- **Data_OExType** – повідомляє, чи закрив користувач діалогове вікно після того, як натиснув посилання на змінення облікового запису

- **Data_ProductName** – повідомляє нам назву продукту пропозиції цифрового вкладення

- **Data_UseInAppRedemption** – повідомляє, яку активацію ми використовуємо: в програмі чи Інтернеті (це стосується лише потоку "Активація для Office").
...
- **DialogRedemption** – сповіщення про відображення діалогового вікна активації функції "Активація для Office".

- **DialogSignIn** – сповіщення про відображення діалогового вікна входу для функції "Активація для Office".
...
- **Tag** – повідомляє, на якому етапі користувач закриває процес активації функції "Активація для Office". Можливі теги:
    ...
    - 0x2370e3a3      Користувач натиснув посилання "Не зараз", яке пропускає пропозицію "Активація для Office" для цього сеансу
    - 0x2370e3a2      Користувач натиснув посилання "Більше не показувати", яке вимикає пропозицію "Активація для Office"


- **UseInAppRedemption** – повідомляє, чи залишиться користувач у програмі для активації чи буде перенаправлений до Інтернету, щоб активувати отриманий PIN-код (внесений раніше).

- **UseModernAFO** – повідомляє, який інтерфейс функції "Активація для Office" використовується: старий чи новий.
...
- **CurrentOOBEMode** – повідомляє про режим попередньої інсталяції (режим першого запуску, наприклад "Активація для Office", OEM тощо).
...
Якщо з якоїсь причини ми не можемо ліцензувати користувача, ми встановлюємо пільговий ключ і надсилаємо сигнал, щоб підтвердити це. Це дуже важливо, щоб визначити, чи перебуває користувач у належному стані та чи всі функції йому доступні. Ця подія використовується для забезпечення справності системи й виконання діагностики, якщо користувач повідомляє про проблему, що виникла в роботі комп’ютера.
...
Ця подія використовується для записування подій для кількісного оцінювання впливу помилки, через яку користувачі сьогодні бачать Null-значення, а не ціну на екрані вибору облікових номерів. Помилка буде діагностуватися додатково, щоб визначити виправлення. 

Збираються перелічені нижче поля:

- **PriceNotFound** — ціни не знайдено в магазині.

- **StoreNotInitilized** — коли магазин не ініціалізовано.
...
Ця подія повідомляє про помилку, виявлену під час спроби підключитися до привілейованого помічника за допомогою підключення XPC. Ми використовуємо цю подію, щоб відстежувати та вирішувати можливі проблеми з інсталяцією MAU.
...
- **Payload** — містить відомості про природу помилки проксі-сервера.
...
Ця подія використовується для перелічення програм, щодо яких програма оновлення Microsoft може керувати оновленнями, кількістю доступних копій і версій та розташуваннями інсталяцій (стандартні або інші).
...
Ця подія повідомляє про різні проблеми із завантаженими файлами, включно з невідповідністю підпису постачальника й значення геша завантаженого файлу. Вона використовується для виявлення проблем, пов’язаних із публікацією набору маніфестів для програм.
...
- **FileHash** – значення геша завантаженого файлу.

- **FileName** – ім’я файлу з невідповідним значенням геша.

- **HashInCatalog** – запис зі значенням геша у відповідному файлі каталогу.

- **HowTocheck** – параметри перевірки оновлень.

- **Payload** – містить інформацію про проблему зі створенням звітів програми.
...
Ця подія реєструє стан помилки, що вказує на неприпустимий завантажений каталог маніфесту. Ця подія використовується, щоб переконатися, що в опублікованих файлах маніфесту немає помилок. 
...
- **CatalogFile** – ім'я файлу каталогу, який спричинив помилку.
...
Ця подія вказує на початок клонування перед оновленням програми. Використовується в поєднанні з подією cloningtask.status для визначення обсягу помилок клонування, а також того, чи слід регулювати функцію клонування на різних каналах аудиторій.
...
Ця подія вказує на те, що оновлення не знайдені в результаті перевірки наявності оновлень. Ця подія використовується для забезпечення наявності необхідних оновлень, оптимізації навантаження на службу та визначення необхідної періодичності перевірки оновлень.
...
Ця подія вказує на те, що була проведена перевірка на наявність оновлень. Ця подія використовується для забезпечення наявності необхідних оновлень, оптимізації навантаження на службу та визначення необхідної періодичності перевірки оновлень.
...
Ця подія вказує на те, що процес перевірки наявності оновлень був скасований (або користувачем, або системою). Ця подія використовується для забезпечення наявності необхідних оновлень, оптимізації навантаження на службу та визначення необхідної періодичності перевірки оновлень.
...
Ця подія вказує на те, що користувач скасував процес перевірки наявності оновлень. Ця подія використовується для забезпечення наявності необхідних оновлень, оптимізації навантаження на службу та визначення необхідної періодичності перевірки оновлень.
...
Ця подія вказує на те, що процедура перевірки оновлень виявила наявні оновлення. Ця подія використовується для забезпечення наявності необхідних оновлень.
...
Ця подія вказує на те, що процедура перевірки оновлень не призвела до оновлення, оскільки програми пристрою оновлені.
...
Ця подія вказує на втрату підключення під час завантаження оновлень. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія вказує на успішність усіх оновлень з поточного пакета. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія повідомляє про помилку з налаштуванням засобу автоматичного оновлення корпорація Майкрософт або з налаштуванням призначеного для користувача сервера в параметрах, або в визначеннях кінцевих точок у помічнику з оновлення у встановлених компонентах засобу автоматичного оновлення. Ми використовуємо цю подію, щоб порадити ІТ-адміністраторам встановити правильні кінцеві точки сервера Manifest.
...
Ця подія означає, що будуть завантажені оновлення. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія вказує на те, що фоновий процес перевіряє наявність оновлень. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія означає, що перевірка оновлення - вимушена. Ця подія використовується для визначення обсягу перевірок вимушених оновлень, які знаходяться за межами нормального циклу перевірок оновлень.
...
Ця подія реєструє неполадки елемента daemon засобу автоматичного оновлення Microsoft під час спроби запуску. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія вказує на те, що помічник сайт Microsoft Update ініціює безшумне оновлення. Ця подія використовується, щоб визначити обсяг оновлень, які застосовуються без втручання користувача для вдосконалення взаємодії з користувачами.
...
- **Payload** – статичний текст *[Це поле вилучено з поточних збірок Office, але воно все одно може відображатися в попередніх збірках.]*

- **PipelineInfo_ClientCountry** – країна або регіон пристрою (на основі IP-адреси).

- **PipelineInfo_ClientIp** – перші 3 октети IP-адреси.

- **Reason** – статичний текст, який показує, що автоматичне оновлення неможливо продовжити, оскільки відкрито інтерфейс користувача.
...
Ця подія означає, що користувач вибирає оновлення в автоматичному режимі. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія означає, що daemon засобу автоматичного оновлення Microsoft припинив роботу в нормальному режимі. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія означає, що елемент daemon засобу автоматичного оновлення Microsoft виявив доступні оновлення. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія означає, що оновлення завантажується. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
- **CustomNotification** – логічне значення, яке вказує, чи використовувалося спеціальне сповіщення.
...
- **Payload** – текст, що вказує на характер події. *[Це поле видалене з поточних збірок Office, але все одно може відображатися в попередніх збірках.]*
...
Ця подія повідомляє про помилку, виявлену під час спроби відобразити відомості про програму в інтерфейсі користувача MAU. Ми використовуємо цю подію, щоб забезпечити належну роботу MAU, відстежувати та усувати несправності.
...
Ця подія вказує на зміну режиму оновлення через інтерфейс користувача. Ця подія використовується, щоб визначити обсяг пристроїв, що переходять з одного режиму до іншого, це допомагає з'ясувати, чому клієнти відмовляються від автоматичних оновлень. 
...
Ця подія вказує на те, що засіб автоматичного оновлення Microsoft (MAU) отримав підтвердження події Apple від зареєстрованої програми, щоб завершити роботу програми та продовжити оновлення програми, яке очікує в черзі. Ця подія наразі використовується, щоб забезпечити майбутні вдосконалення, пов'язані з мінімізацією переривання роботи користувачів під час оновлення програми. 
...
Ця подія вказує на те, що засіб автоматичного оновлення Microsoft (MAU) отримав код відповіді на подію Apple від зареєстрованої програми, щоб завершити роботу програми та продовжити оновлення програми, яке очікує в черзі. Ця подія наразі використовується, щоб забезпечити майбутні вдосконалення, пов'язані з мінімізацією переривання роботи користувачів під час оновлення програми. 
...
Ця подія реєструє стан двійкового підпису коду ОС. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє стан елемента daemon засобу автоматичного оновлення Microsoft. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
- **BundleReachable** – логічне значення, яке вказує, чи виникала проблема з доступом до ресурсів програми Microsoft AutoUpdate.

- **Channel** – уподобання для аудиторії.

- **Codesigned** – логічне значення, яке вказує, чи спільна розробка помічника з оновлення була правильною.
...
- **Exists** – логічне значення, яке вказує, чи помічник з оновлення існує на диску.

- **HowTocheck** – параметри перевірки оновлень.

- **Payload** – вказує, чи компонент Daemon існує в потрібному розташуванні та чи він має підпис коду. *[Це поле видалене з поточних збірок Office, але все одно може відображатися в попередніх збірках.]*
...
Ця подія реєструє стан помічника служби автоматичного оновлення Microsoft. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія вказує на те, що під час повторної спроби інсталяції сталися помилки. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє помилки взаємодії між процесами (зв’язок із помічником засобу автоматичного оновлення Microsoft). Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє невдалу повторну спробу. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє успішну інсталяцію оновлення після повторної спроби. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
- **Payload** – текст, що позначає успішність операції. *[Це поле видалене з поточних збірок Office, але все одно може відображатися в попередніх збірках.]*
...
- **Success** — логічне значення, що вказує на успішність операції.
...
Ця подія вказує на запуск засобу автоматичного оновлення Microsoft чи помічника сайт Microsoft Update. Крім того, вона використовується, щоб повідомити про проблеми, виявлені під час процесу запуску, а також метод звітування, що використовується у випадку помічника сайт Microsoft Update.
...
Ця подія означає, що оновлення були завантажені. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє список оновлень у черзі, інсталяцію яких завершено. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє список оновлень у черзі. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
- **Duration** — текст із зазначенням тривалості відкладання.
...
Ця подія реєструє умову вимкнення функції "Інсталяція на клон" для певного оновлення. Ця подія використовується для відстеження справності функції "Інсталяція на клон" і надання покращених послуг.
...
- **Причина** – причина вимкнення інсталяції на клон для цього оновлення.
...
- **Примусове оновлення** – рядкове позначення того, чи ІТ-адміністратор примусово встановив оновлення
...
Ця подія реєструє спроби реєстрації програми та результат або причину невдачі. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія вказує на завантаження агента запуску. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє помилки взаємодії з мережею доставки вмісту. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія повідомляє про помилку, що вказує на відсутність файлів маніфесту, які відповідають ідентифікатору програми, який отримано з веб-відповіді. Ця подія використовується для дослідження повідомленої помилки.
...
Ця подія повідомляє про помилку, яка вказує на те, що ідентифікатор програми, отриманий з веб-відповіді має неправильний формат. Ця подія використовується для дослідження повідомленої помилки.
...
Ця подія реєструє ініціацію перевірки наявності оновлень. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє доступність мережі. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
- **ServerReacheable** — логічне зазначення доступності мережі.

    
### <a name="updatemanagerupdatespending"></a>updatemanager.updatespending

Ця вказує на те, що оновлення знайдені і очікують інсталяції. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія означає, що виявлено елементи для автоматичного оновлення. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє запити до служби автоматичного оновлення Microsoft, час відгуку та помилки. Ця подія використовується для забезпечення належного виконання процесу оновлення та для усунення помилок.
...
Ця подія реєструє фіксує результат синхронізації розділу. Вона використовується ля з’ясування кількості унікальних об’єктів синхронізації під час обчислення оцінки синхронізації OneNote. Вона також використовується для дошки продуктивності сучасної синхронізації OneNote.
...
Ця подія записується, якщо не вдалося синхронізувати Quick Notes для користувача, під час першого запуску на пристрої. Це характерно для сценарію першого запуску.
...
Ця подія допомагає відстежувати використання запуску календарних зустрічей у різних кінцевих точках. Ця подія дає змогу виявляти дві речі: коли зустріч запуститься через Skype для бізнесу і через Teams, якщо програму Teams інстальовано.

Збираються перелічені нижче поля: 

- **account** — гешований обліковий запис, який виконав дію.

- **action_type** — виконана дія, наприклад запуск наради або інсталяція програми.

- **application** — програма, яку запущено через точне посилання, як-от Teams чи Skype для бізнесу.

- **context** - Досвід переходу до програми, наприклад, office_union - слово, office_union - excel тощо.

- **source** - Походження дії, наприклад, ініційованої користувачем, автоматично клієнтом тощо.
...
Метадані на основі сеансів, коли користувачеві відображається інтерфейс користувача Paywall. Ця подія надає корпорація Майкрософт відомості про шлях взаємодії користувача та версії пристрою й ОС, які використовує користувач, і допомагає приймати рішення щодо інвестицій задля покращення роботи в цих областях.
...
Збираються такі поля:
  ...
  - **AffectedProcessAppBuild** – ідентифікатор версії збірки для процесу, на який впливає проблема. *[Ця поле вилучено з поточних збірок Office, але воно все одно може відображатися в попередніх збірках.]*

  - **AffectedProcessAppBuildRevision** – ідентифікатор редакції збірки для процесу, на який впливає проблема. *[Ця поле вилучено з поточних збірок Office, але воно все одно може відображатися в попередніх збірках.]*

  - **AffectedProcessAppMajorVer** – ідентифікатор проміжної версії для процесу, на який впливає проблема. *[Ця поле вилучено з поточних збірок Office, але воно все одно може відображатися в попередніх збірках.]*

  - **AffectedProcessAppMinorVer** – ідентифікатор номера проміжної версії для процесу, на який впливає проблема. *[Ця поле вилучено з поточних збірок Office, але воно все одно може відображатися в попередніх збірках.]*
  ...
  - **AffectedProcessAppVersion** Ідентифікатор версії відповідного процесу.
...
- **CallStack** — внутрішній стек викликів Microsoft, що викликає аварійне завершення роботи.
...
Збираються такі поля:
  ...
  - **CallStack** — внутрішній стек викликів корпорація Майкрософт, що викликає аварійне завершення роботи.
...
Зменшення показників DQ. Потрібні для підтвердження надійності всіх інших даних.
